...
export function LOCCard({ data }: { data: GitHubData }) {
  const { stats } = data
  const loc = stats.linesOfCode || 0
  ...
  return (
    <div className="flex items-center justify-center w-full h-full min-h-[600px] bg-[#F3F3F1] p-4 rounded-3xl">
      {/* Card Container */}
      <div className="relative w-full h-[620px] md:h-auto max-w-[400px] aspect-[9/14] bg-[#1A1A1A] text-[#F3F3F1]  flex flex-col items-center justify-between p-6 rounded-3xl shadow-2xl">
        ...
        <div className="relative z-30 flex-1 w-full flex flex-col items-center justify-center gap-5">
          ...
          <div className="text-center space-y-2 max-w-[90%]">
             <p className="text-md text-[#f3f3f1ca] font-medium leading-relaxed font-montserrat">
               You wrote <span className="text-white font-bold">{loc.toLocaleString()}</span> lines of code. 
               That&apos;s <span className="text-white font-bold">{commits}</span> commits. Awesome.
             </p>
          </div>
